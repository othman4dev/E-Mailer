        ...
        <div class="d-flex  justify-content-center">
            <div class="cardForm shadow p-5 m-3 ">
                ...
                <form method="POST" action="/register" name="registerForm"
                    ...
                    @csrf <!-- Ensure this is properly processed on the server-side -->
                    <label class="text-secondary">Your Name
                        <input required type="text" name="name" placeholder="Your Name ..."
                            class="form-control mt-1">
                        <span id='errorMessage3' class='text-danger' style='display:none'>Please enter your name.</span>
                    </label>
                    <label class="text-secondary">Your Email
                        <input required type="email" name="email" placeholder="Your Email ..."
                            class="form-control mt-1">
                        <span id='errorMessage1' class='text-danger' style='display:none'>Please enter a valid
                            email.</span>
                    </label>
                    <label class="text-secondary">Your Password
                        <input required type="password" name="password" class="form-control mt-1">
                        <span id='errorMessage2' class='text-danger' style='display:none'>Please enter a
                            password.</span>
                    </label>
                    <label class="text-secondary">
                        <input required type="checkbox" name="terms" class="form-check-input"> I agree and accept the
                        <a href="#" class="blueColor no-underline">terms and conditions</a>
                    </label>
                    <button type="submit" id="registerSubmit" class="btn btn-primary">Create Account</button>
                    <p class="text-secondary">Already have an account? <a href="<?= $_ENV['APP_URL'] . '/login' ?>"
                            class="blueColor no-underline"> Log in</a></p>
                </form>
